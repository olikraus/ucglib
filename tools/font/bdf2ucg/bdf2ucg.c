...
#define BDF2UCG_COMPACT_OUTPUT
#define BDF2UCG_VERSION "1.01"
//#define VERBOSE
...
int map_ucg_to_unicode[256];

int map_UnicodeToU8G(int unicode)
{
  int i;
  for( i = 0; i < 256; i++ )
    if ( map_ucg_to_unicode[i] == unicode )
      return i;
  return 0;
}
...
{
  int i;
  map_ucg_to_unicode[0] = 0;
  for( i = 0; i < 256; i++ )
    map_ucg_to_unicode[i] = i;
}
...
void map_UpperLowerPage(int lower_page, int upper_page, int shift, int upper_shift)
{
  ...
  for( i = 128; i < 256; i++ )
  {
    ...
    if ( i+upper_shift < 256 )
      map_ucg_to_unicode[i+upper_shift] = encoding;
  }

  for( i = 0; i < 256; i++ )
  {
    tmp[i] = map_ucg_to_unicode[i];
  }

  for( i = 0; i < shift; i++ )
  {
    map_ucg_to_unicode[i] = -1;
  }
  
  for( i = shift; i < 256; i++ )
  {
    map_ucg_to_unicode[ i ] = tmp[(i + 256 - shift) % 256];
  }
  /*
  printf("map_ucg_to_unicode[ 32 ] = %d\n", map_ucg_to_unicode[ 32 ]);
  printf("map_ucg_to_unicode[ 33 ] = %d\n", map_ucg_to_unicode[ 33 ]);
  */
}
